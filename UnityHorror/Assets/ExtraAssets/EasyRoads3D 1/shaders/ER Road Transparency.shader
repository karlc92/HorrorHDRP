	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord47 = packedInput.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack58.z = lerp( 1, unpack58.z, saturate(_BumpScale) );
				
				float2 uv_Metallic = packedInput.ase_texcoord5.xy * _Metallic_ST.xy + _Metallic_ST.zw;
				float4 tex2DNode48 = tex2D( _Metallic, uv_Metallic );
				
				surfaceDescription.BaseColor = ( tex2DNode42 * _Color ).rgb;
				surfaceDescription.Normal = unpack58;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( tex2DNode48.r * _MainMetallicPower1 );
				...
				surfaceDescription.Smoothness = ( tex2DNode48.a * _MainSmoothnessPower1 );
				surfaceDescription.Occlusion = ( tex2DNode48.g * _OcclusionStrength1 );
				surfaceDescription.Alpha = ( ( tex2DNode42.a * _Color.a ) * packedInput.ase_color.a );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 texCoord47 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack58.z = lerp( 1, unpack58.z, saturate(_BumpScale) );
		...
		Pass
		{
			...
			UNITY_SETUP_INSTANCE_ID(packedInput);
				...
				float2 texCoord47 = packedInput.ase_texcoord3.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack58.z = lerp( 1, unpack58.z, saturate(_BumpScale) );
		...
		Pass
		{
						...
						)
			{
				...
				float2 texCoord47 = packedInput.ase_texcoord7.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord47 ), _BumpScale );
				unpack58.z = lerp( 1, unpack58.z, saturate(_BumpScale) );
