	...
	SubShader
	{
		...
		Pass
		{
						...
						)
			{
				...
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord41 ), _BumpScale );
				...
				float4 tex2DNode46 = tex2D( _Metallic, uv_Metallic );
				
				surfaceDescription.BaseColor = ( tex2DNode48 * _Color ).rgb;
				surfaceDescription.Normal = unpack58;
				surfaceDescription.BentNormal = float3( 0, 0, 1 );
				surfaceDescription.CoatMask = 0;
				surfaceDescription.Metallic = ( tex2DNode46.r * _MainMetallicPower );
				...
				surfaceDescription.Smoothness = ( tex2DNode46.a * _MainSmoothnessPower );
				surfaceDescription.Occlusion = ( tex2DNode46.g * _OcclusionStrength );
				surfaceDescription.Alpha = tex2DNode48.a;
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float2 texCoord41 = packedInput.ase_texcoord2.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack58 = UnpackNormalScale( tex2D( _BumpMap, texCoord41 ), _BumpScale );
				unpack58.z = lerp( 1, unpack58.z, saturate(_BumpScale) );
